"use client"

import { useEffect, useState } from "react"
import { trackEvent } from "@/lib/analytics"
import { SITE_CONFIG } from "@/lib/site-contact"
import styles from "./StickyContactButton.module.css"

export function StickyContactButton() {
  const [visible, setVisible] = useState(false)
  const [mobile, setMobile] = useState(false)

  useEffect(() => {
    const mq = window.matchMedia("(max-width: 768px)")
    function update() {
      setMobile(mq.matches)
    }
    update()
    mq.addEventListener("change", update)
    function onScroll() {
      setVisible(window.scrollY > 300)
    }
    window.addEventListener("scroll", onScroll, { passive: true })
    onScroll()
    return () => {
      mq.removeEventListener("change", update)
      window.removeEventListener("scroll", onScroll)
    }
  }, [])

  if (!mobile || !visible) return null

  return (
    <div className={styles.stickyContact}>
      <div className={styles.contactButtons}>
        <a
          href={`tel:${SITE_CONFIG.phoneTel}`}
          className={`${styles.contactBtn} ${styles.phone}`}
          onClick={() => trackEvent("sticky_phone_click", { phone_number: SITE_CONFIG.phone, page: "home" })}
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
          </svg>
          Call
        </a>
        <a
          href={`sms:${SITE_CONFIG.phoneTel}`}
          className={`${styles.contactBtn} ${styles.text}`}
          onClick={() => trackEvent("sticky_text_click", { phone_number: SITE_CONFIG.phone, page: "home" })}
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden>
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
          </svg>
          Text
        </a>
      </div>
    </div>
  )
}
